...
def _ownership_and_gradients(components: list[list[int]], masks: np.ndarray, logits: np.ndarray, associations: np.ndarray, gt: np.ndarray) -> dict[str, Any]:
    ...
    for component_id, nodes in enumerate(components):
        ...
        for node in nodes:
            ...
            other_logits = logits[member]
            other = other_logits.max(axis=0)
            margin = logits[node] - other
            # A candidate has no logit outside its source crop.  Such values
            # are represented as -inf for partition CE only; they are not a
            # valid ownership comparison or margin observation.
            comparison_domain = target_pixels & np.isfinite(logits[node]) & np.any(np.isfinite(other_logits), axis=0)
            wrong = comparison_domain & (other > logits[node])
            boundary = _boundary_inside(gt == gt_id)
            comparison_domain_pixels += int(comparison_domain.sum())
            margin_values.extend(margin[comparison_domain].astype(float).tolist())
            ...
            negative = comparison_domain & (margin < 0)
            nonnegative = comparison_domain & ~negative
            if negative.any():
        ...
        for image_id, candidates in candidates_by_image.items():
            ...
            for component in components:
                for node in component:
                    ...
                    if other:
                        other_logits = logits[other]
                        wrong |= (gt == associations[node]) & np.isfinite(logits[node]) & np.any(np.isfinite(other_logits), axis=0) & (np.max(other_logits, axis=0) > logits[node])
